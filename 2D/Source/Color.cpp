...
color_t AlphaBlend(const color_t& src, const color_t& dest) {
	...
	color_t color;
	color.r = (src.r * alpha + dest.r * inv_alpha) / 255;
	color.g = (src.g * alpha + dest.g * inv_alpha) / 255;
	color.b = (src.b * alpha + dest.b * inv_alpha) / 255;
	color.a = src.a;
...
color_t AdditiveBlend(const color_t& src, const color_t& dest) {
	...
	color.g = std::min(src.g + dest.b, 255);
	color.b = std::min(src.g + dest.g, 255);
	color.a = src.a;
...
color_t MultiplyBlend(const color_t& src, const color_t& dest)
{
	...
	color.g = (src.g * dest.b) >> 8;
	color.b = (src.g * dest.g) >> 8;
	color.a = src.a;
